...
def train_accuracy_predictor(audio_models, video_models, fusion_models, test_loader, device, accuracy_table):
    ...
    for epoch in range(50):
                ...
                for video_fps in [20, 25, 29]:
                    for a_idx, audio_model in enumerate(audio_models):
                        for audio_chunk_size in [1200, 1000, 800]:
                            ...
                            acc_loss = F.mse_loss(acc_pred, accuracy_labels) * acc_loss_weight
                            
                            logits_loss = F.kl_div(
                                F.log_softmax(logits_pred, dim=1),
                                F.log_softmax(logits_ground_truth.detach(), dim=1),
                                reduction='batchmean'
                            ) * logits_loss_weight
        ...
        if avg_loss < min_loss:
            ...
            # Save the model
            torch.save(predictor.state_dict(), "./checkpoints/accuracy_predictor/accuracy_predictor_logits.pth")
...
if __name__ == '__main__':
    ...
    accuracy_table = json.load(open("./accuracy_table.json", "r"))
