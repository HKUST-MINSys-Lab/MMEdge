...
from models.Audio_ResNet import AudioEncoder, AudioModel, AudioModelBaseline
...
def evaluate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for _, audio_inputs, labels in progress_bar:
            ...
            # indices = evenly_spaced_indices(total_range=19456, num_indices=18000)

            # audio_inputs = audio_inputs[:, indices]
...
if __name__ == '__main__':
    ...
    num_classes = 500
    # model_config = 'large'
    # model = AudioModel(model_config, chunk_size=800, chunk_interval=800)
    model = AudioModelBaseline(num_classes=num_classes)
    # model = AudioModelBaseline()
    # model.apply(init_weights)
    # model = Video_Classification_3D(feature_dim=512, num_classes=num_classes)
    # audio_filtered_dict = torch.load(f'checkpoints/audio/audio_{model_config}.pth')
    audio_filtered_dict = torch.load('checkpoints/LRW/baselines/audio_baseline.pth')
    # audio_pretrained_dict = torch.load(f'checkpoints/baselines/audio_baseline.pth')
    audio_filtered_dict = {k.replace('module.', ''): v for k, v in audio_filtered_dict.items()}
    # audio_filtered_dict = {k: v for k, v in audio_filtered_dict.items() if not k.startswith('fc.')}
    model.load_state_dict(audio_filtered_dict)
