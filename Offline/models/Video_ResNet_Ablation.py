...
class FrameLevelTemporalShift(nn.Module):
    def __init__(self, n_div=4):
        super().__init__()
        self.n_div = n_div  # 分组数（保留参数以对齐接口）
    ...
    def forward(self, x):  # x: (B, C, T)
        ...
        # 帧级别位移：对每个通道的所有帧进行位移
        out[:, :, 1:] = x[:, :, :-1]  # 所有通道前移1帧
        out[:, :, :-1] += x[:, :, 1:]  # 所有通道后移1帧（双向混合）

        if T > 2:  # 多帧时增加更长时序位移
            out[:, :, 2:] += x[:, :, :-2]  # 前移2帧
            out[:, :, :-2] += x[:, :, 2:]   # 后移2帧

        return out / 2.0  # 归一化（避免数值爆炸）
...
class Video_ResNet_P3D_Without_Shift(nn.Module):
    def __init__(self, model, feature_dim=2048, num_classes=50):
        ...
        # self.temporal_shift = FrameLevelTemporalShift(n_div=4)
...
class Video_ResNet_P3D_Without_Diff(nn.Module):
    def __init__(self, model, feature_dim=2048, num_classes=50):
        super().__init__()
        self.feature_dim = feature_dim
        self.spatial_encoder = Spatial_Encoder(model, feature_dim)  # (B*T, 2048, 1, 1)
...
class Video_ResNet_P3D_Without_SE(nn.Module):
    def __init__(self, model, feature_dim=2048, num_classes=50):
        super().__init__()
        self.feature_dim = feature_dim
        self.spatial_encoder = Spatial_Encoder(model, feature_dim)  # (B*T, 2048, 1, 1)
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        ...
        feat = self.temporal_shift(feat)
        feat = self.temporal_conv(feat)
        pooled = feat.mean(dim=-1, keepdim=True).squeeze(-1) # (B, 512)
...
class Video_ResNet_Baseline(nn.Module):
    def __init__(self, model, feature_dim=2048, num_classes=50):
        super().__init__()
        self.feature_dim = feature_dim
        self.spatial_encoder = Spatial_Encoder(model, feature_dim)

        self.temporal_conv = SimpleTemporalConv(in_channels=512, out_channels=512)
    ...
    def forward(self, x):  # x: (B, T, C, H, W)
        ...
        pooled = feat.mean(dim=2)  # temporal mean pooling → (B, 512)
        logits = self.classifier(pooled)  # (B, num_classes)
        return logits
    

class Video_ResNet_Baseline_Encoder(nn.Module):
    def __init__(self, model, feature_dim=2048, num_classes=50):
        super().__init__()
        self.feature_dim = feature_dim
        self.spatial_encoder = Spatial_Encoder(model, feature_dim)
...
if __name__ == '__main__':
    model, feature_dim = get_resnet_backbone('resnet50', pretrained=True)
    model = Video_ResNet_P3D_Frame_Shift(model, feature_dim=feature_dim, num_classes=50)
    x = torch.randn(2, 16, 3, 224, 224)
    out = model(x)
    print(out.shape)  # (2, 50)
